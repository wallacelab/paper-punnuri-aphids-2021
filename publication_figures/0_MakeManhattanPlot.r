...
plot_manhattan = function(trait_name, glm_file, farm_file, args){
    ...
    myplot = ggplot(farm, mapping=aes(x=get_offset(Chr, Pos), y=rmip)) + 
        #geom_point(shape=25, size=2, color="black", fill="black") +  # Make a scatterplot
        #scale_color_manual(values=chrom_colors) +
    ...
    if(nrow(farm.all) > 0){
        ...
        for(mychrom in rownames(offsets)){
            ...
            #mypoints$mycolor=ifelse(as.numeric(mypoints$Chr)%%2==0, yes="red", no="blue")
            
            # Add to plot
            mycolor = ifelse(as.numeric(mychrom) %% 2 ==0, yes=chrom_colors[1], no=chrom_colors[2])
            myplot = myplot +
                #geom_line(data=mypoints, mapping=aes(y=rmip.plot), color=cum_color, size=0.35)
                geom_line(data=mypoints, mapping=aes(y=cum_rmip), color=mycolor, size=0.35) 
        }
        
        # Add horizontal reference line
        #ref_line_y = rescale(args$rmip_cutoff, from=c(0, max(farm.all$rmip)), to=c(0, max(glm$log.p)))
        myplot = myplot +
            geom_hline(yintercept=args$rmip_cutoff, color=cum_color, size=0.35, linetype="dashed")
    }
    
    myplot = myplot + 
        geom_point(shape=25, size=2, color="black", fill="black") 
